...
public class Autobus {
    ...
    private boolean isPassengerInBus(Passenger passenger) {
        ...
        return false;
   // }
    //private boolean dropPassenger(Passenger passenger) {
     //   for (int i = 0; i < countPassengers ; i++) {
   //       if (passengers[i].getId() ==passenger.getId()) {
   //             for (int x = i;x < countPassengers -1; x++) { //сдвинуть елементы массива влево,чтобы удалить пассажира
   //                 passengers[x] = passengers[x + 1];
    //            }
   //             passengers[countPassengers -1] = null;//обнуляем посдедний индекс
  //             countPassengers--;//уменьшаем количество пассажиров
   ...
   // }
    ...
    public String toString() {
        StringBuilder dr = new StringBuilder("Autobus: {");
        dr.append("id:" ).append(id);
        dr.append("capacity:" ).append(capacity);
        dr.append("driver:" ).append(driver);
        dr.append("autopilot:" ).append(autopilot);
        dr.append("}");
